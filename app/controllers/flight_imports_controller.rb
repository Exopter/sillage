class FlightImportsController < ApplicationController
  ...
  def create
    ...
    respond_to_upload_success(flights_path(recording: flight_import.id), t(".queued"))
  ...
  def show
    ...
    if @flight_import.imported? && !@flight_import.set_aside? && @flights.one?
      ...
      redirect_to flights_path(recording: @flight_import.id, filter: @flight_import.set_aside? ? "set_aside" : "flights")
    end
  end
  ...
  def include_in_flights
    ...
    redirect_to flights_path(recording: flight_import.id), notice: "Recording included in Flights."
  rescue ExoFdr::Error => error
    redirect_to flights_path(recording: flight_import.id), alert: error.message
  rescue ActiveJob::EnqueueError
    redirect_to flights_path(recording: flight_import.id), alert: "The recording could not be queued. Please try again."
  end
  ...
  def destroy
    ...
    redirect_to flights_path(recording: flight_import.id), alert: "The recording could not be deleted. Please try again."
  end
