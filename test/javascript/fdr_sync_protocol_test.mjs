...
assert.match(connectivitySource, /renderWifiSynchronizationProgress\(/)
...
assert.match(connectivityViewSource, />Recording</)
assert.match(connectivityViewSource, /role="switch"/)
assert.match(connectivityViewSource, /signal-fdr-wifi-settings-link/)
assert.match(connectivityViewSource, /Advanced diagnostics/)
assert.doesNotMatch(connectivityViewSource, /Persistent control/)
assert.doesNotMatch(connectivityViewSource, /Status update interval/)
assert.doesNotMatch(connectivitySource, /client\.config\(\)/)
assert.doesNotMatch(connectivitySource, /BleUuid\.config/)
assert.match(connectivitySource, /Paused while USB-C is connected\./)
assert.match(connectivitySource, /Waiting for recorder confirmation…/)
assert.doesNotMatch(connectivitySource, /Saved choice:/)
...
const toolsAvailabilitySource = connectivitySource.split("  updateToolsAvailability() {", 2)[1]
  .split("\n  updateUsbActions() {", 1)[0]
assert.doesNotMatch(toolsAvailabilitySource, /recorderToolsTarget\.open\s*=/)
