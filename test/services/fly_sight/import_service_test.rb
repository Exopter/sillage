...
module FlySight
  class ImportServiceTest < ActiveSupport::TestCase
    ...
    test "imports paired FlySight V2 files" do
      ...
      assert_equal 8, flight.track_points.count
      assert_equal 8, flight.sensor_samples.count
      assert_equal 8, flight.sample_count
      assert flight.exit_at
      assert flight.opening_at
      assert flight.landing_at
    end
    ...
    test "imports a zipped FlySight V2 session folder" do
      ...
      assert_equal "imported", import.status
      assert_equal 1, import.flights.count
      assert_equal 8, import.flights.first.sensor_samples.count
    ensure
      archive&.close!
    end
    ...
    test "imports a bulk FlySight ZIP with date and time folders" do
      ...
      assert_equal "imported", import.status
      assert_equal 2, import.flights.count
      assert_equal 2, import.details.fetch("sessions_count")
      assert_equal [ 8, 8 ], import.flights.order(:id).map { |flight| flight.track_points.count }
    ensure
      archive&.close!
    end
