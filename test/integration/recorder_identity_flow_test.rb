...
class RecorderIdentityFlowTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @assembly = Assembly.create!(name: "Physical recorder", hardware_definition: create_hardware_definition)
  ...
  test "Forge shows the physical recorder first and the interchangeable ECU second" do
    label = "ExoFDR · S/N #{@assembly.serial_number}"
    get forge_fdrs_path
  ...
  test "activity snapshots retain the recorder identity after reassignment" do
    ...
    @ecu.part.remove_from_assembly!
    other = Assembly.create!(name: "Other recorder", hardware_definition: @assembly.hardware_definition)
    @ecu.part.install_in!(other)
    assert_equal "ExoFDR · S/N #{@assembly.serial_number}", activity.reload.recorder_label
  ...
  test "historical identity resolution never uses the current assembly" do
    ...
    assert_equal "ExoFDR · S/N #{@assembly.serial_number}", recording.recorder_label
    recording.details = {}
